...
//商家商品清單
	...
	if($link->connect_error){ //連線錯誤
		...
		$sql="SELECT produce.*, company.company_name 
		FROM produce,company 
		where produce.company_id = $company_id 
		AND company.company_id=$company_id
		AND produce.total_search>=1 
		ORDER BY produce.total_search DESC";
